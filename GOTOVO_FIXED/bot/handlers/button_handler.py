...
from bot.config.config import load_config, get_current_rates, is_admin
...
BUTTON_MAP = {
    ...
    "💬 Управление текстами": {
        "action": "manage_texts",
        "requires_admin": True,
        "description": "Управление текстовыми сообщениями бота"
    },
    "🔘 Управление кнопками": {
        "action": "manage_buttons",
        "requires_admin": True,
        "description": "Управление кнопками интерфейса"
    },
    "💱 Управление валютами": {
        "action": "currency_management",
        "requires_admin": True,
        "description": "Управление поддерживаемыми валютами"
    },
    "🔔 Уведомления": {
...
async def process_button(update: Update, context: ContextTypes.DEFAULT_TYPE, button_text: str) -> bool:
    ...
    """
    # Проверяем, есть ли кнопка в словаре
    if button_text not in BUTTON_MAP:
        return False
        
    button_info = BUTTON_MAP[button_text]
    user_id = update.effective_user.id
    
    # Проверяем права доступа для кнопок, требующих особых прав
    ...
    if button_info.get("requires_operator", False) and not (await check_operator(user_id) or await check_admin(user_id)):
        await update.message.reply_text(
            "❌ У вас нет прав оператора для выполнения этого действия."
        )
        return True
